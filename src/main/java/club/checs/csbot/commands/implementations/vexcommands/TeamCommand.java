...
public class TeamCommand extends VexQueryCommand {
    ...
    public void onCommand(CommandCall call) {
        ...
        String description = "[Vex Team " + call.getArg("team") + " - " + teamData.get("team_name").getAsString() +
                "](https://vexdb.io/teams/view/" + call.getArg("team") + ")\n";
        ...
        // Awards, inline
        builder.appendField("Awards", getResults(query("get_awards", "&team=" + call.getArg("team"))).size()
                + "", true);


        // Last tournament, not inline
        JsonArray events = getResults(query("get_events", "&team=" + call.getArg("team")));
        if (events.size() > 0) {
            ...
            for (int i = 0; i < events.size(); i++) {
                ...
                JsonArray resultArray = getResults(query("get_rankings", "&team=" + call.getArg("team") + "&sku="
            ...
            JsonObject skills = getJsonByType(2, getResults(query("get_skills", "&team=" + call.getArg("team") + "&sku="
                    + latest.get("sku").getAsString())));
            if (skills != null) {
